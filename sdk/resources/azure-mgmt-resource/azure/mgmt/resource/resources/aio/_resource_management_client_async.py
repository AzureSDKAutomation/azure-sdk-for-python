...
class ResourceManagementClient(MultiApiClientMixin, _SDKClient):
    ...
    def tags_operation(self):
        """Instance depends on the API version:
           ...
           * 2020-06-01: :class:`TagsOperationOperations<azure.mgmt.resource.v2020_06_01.aio.operations_async.TagsOperationOperations>`
        """
        api_version = self._get_api_version('tags_operation')
        if api_version == '2020-06-01':
            from ..v2020_06_01.aio.operations_async import TagsOperationOperations as OperationClass
        else:
            raise NotImplementedError("APIVersion {} is not available".format(api_version))
        return OperationClass(self._client, self._config, Serializer(self._models_dict(api_version)), Deserializer(self._models_dict(api_version)))
