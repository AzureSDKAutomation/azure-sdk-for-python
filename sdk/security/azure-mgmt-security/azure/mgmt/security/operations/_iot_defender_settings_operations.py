...
class IotDefenderSettingsOperations(object):
    ...
    def download_on_premise_activation(
            self, custom_headers=None, raw=False, **operation_config):
        ...
        """
        # Construct URL
        url = self.download_on_premise_activation.metadata['url']
        path_format_arguments = {
            'subscriptionId': self._serialize.url("self.config.subscription_id", self.config.subscription_id, 'str', pattern=r'^[0-9A-Fa-f]{8}-([0-9A-Fa-f]{4}-){3}[0-9A-Fa-f]{12}$')
        }
        url = self._client.format_url(url, **path_format_arguments)
        ...
        if raw:
            client_raw_response = ClientRawResponse(None, response)
            return client_raw_response
    download_on_premise_activation.metadata = {'url': '/subscriptions/{subscriptionId}/providers/Microsoft.Security/iotDefenderSettings/default/downloadOnPremiseActivation'}
