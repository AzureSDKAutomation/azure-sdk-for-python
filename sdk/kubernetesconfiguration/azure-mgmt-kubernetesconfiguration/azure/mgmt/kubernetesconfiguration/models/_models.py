...
class ResourceProviderOperationList(Model):
    ...
    :type value:
     list[~azure.mgmt.kubernetesconfiguration.models.ResourceProviderOperation]
...
class SourceControlConfigurationList(Model):
    """Result of the request to list Source Control Configurations.  It contains a
    list of SourceControlConfiguration objects and a URL link to get the next
    set of results.

    Variables are only populated by the server, and will be ignored when
    sending a request.

    :ivar value: List of Source Control Configurations within a Kubernetes
     cluster.
    :vartype value:
     list[~azure.mgmt.kubernetesconfiguration.models.SourceControlConfiguration]
