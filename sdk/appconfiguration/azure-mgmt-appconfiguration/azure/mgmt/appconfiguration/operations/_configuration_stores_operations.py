...
class ConfigurationStoresOperations(object):
    ...
    def list_keys(
            self, resource_group_name, config_store_name, skip_token=None, filter=None, custom_headers=None, raw=False, **operation_config):
        ...
        :param skip_token: A skip token is used to continue retrieving items
         ...
         for subsequent calls.
        :type skip_token: str
        :param filter: The filter to apply on the access keys. For example,
         you can use "$filter=readOnly eq true" to retrieve only read-only
         access keys. Properties that support filtering include 'name' and
         'readOnly'.
        :type filter: str
        :param dict custom_headers: headers that will be added to the request
        :param bool raw: returns the direct response alongside the
