...
class CachesOperations(object):
    ...
    def force_gsi(
            self, resource_group_name, cache_name, comment=None, custom_headers=None, raw=False, **operation_config):
        ...
        if comment is not None:
            comment1 = models.ForceGsiProperties(comment=comment)

        # Construct URL
        url = self.force_gsi.metadata['url']
        path_format_arguments = {
            'resourceGroupName': self._serialize.url("resource_group_name", resource_group_name, 'str'),
            'subscriptionId': self._serialize.url("self.config.subscription_id", self.config.subscription_id, 'str'),
            'cacheName': self._serialize.url("cache_name", cache_name, 'str', pattern=r'^[-0-9a-zA-Z_]{1,80}$')
        ...
        if comment1 is not None:
            body_content = self._serialize.body(comment1, 'ForceGsiProperties')
        else:
            body_content = None
        ...
        if raw:
            client_raw_response = ClientRawResponse(None, response)
            return client_raw_response
    force_gsi.metadata = {'url': '/subscriptions/{subscriptionId}/resourcegroups/{resourceGroupName}/providers/Microsoft.StorageCache/caches/{cacheName}/forceGsi'}
